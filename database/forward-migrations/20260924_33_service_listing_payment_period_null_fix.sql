...
-- =========================================================
-- M33 - Service listing payment publication period null fix
--
-- Root cause:
--   apply_service_listing_payment_status() calculated
--   v_starts_at / v_ends_at, then reused those variables
--   as SELECT INTO targets while checking whether a
--   publication period already existed.
--
--   On a first successful payment there is no period row.
--   SELECT INTO therefore cleared those variables to NULL,
--   causing the subsequent INSERT to violate starts_at
--   NOT NULL and roll back the entire payment application.
--
-- Fix:
--   Look up only the period id first.
--   Read its interval only when an existing row is found.
--   Otherwise preserve the newly calculated interval.
--
-- Secondary defect:
--   Unqualified provider_id predicates in Service Listing
--   lifecycle updates were ambiguous with the PL/pgSQL
--   provider_id identifier.
--
--   Qualify those predicates against service_listings so
--   payment application can continue through every listing
--   lifecycle branch.
--
-- Scope:
--   apply_service_listing_payment_status() only.
-- =========================================================
...
begin
  ...
  then
    ...
    then
      ...
      where
        ...
        and service_listings.provider_id =
  ...
  if found then
    /*
     * Existing publication period:
     * reuse its authoritative interval.
     *
     * starts_at / ends_at are read only after existence
     * has been established, so a no-row lookup cannot
     * clear the newly calculated interval.
     */
    select
      pp.starts_at,
      pp.ends_at
    ...
    /*
     * New publication period:
     * keep the v_starts_at / v_ends_at values calculated
     * immediately before the existing-period lookup.
     */
    insert into
