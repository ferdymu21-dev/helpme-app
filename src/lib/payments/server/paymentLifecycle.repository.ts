...
export async function listPendingPaymentCandidates(
  ...
  const { data: taskPayments, error: taskPaymentsError } = await adminSupabase
    .from("task_payments")
    .select(
      `
        midtrans_order_id,
  ...
  const { data: servicePayments, error: servicePaymentsError } =
    await adminSupabase
  ...
  for (const donation of donations ?? []) {
    if (
      typeof donation.midtrans_order_id !== "string" ||
      typeof donation.payment_expires_at !== "string"
    ) {
      continue;
    }
  ...
  for (const payment of taskPayments ?? []) {
    if (
      typeof payment.midtrans_order_id !== "string" ||
      typeof payment.payment_expires_at !== "string"
    ) {
      continue;
    }
  ...
  candidates.sort((left, right) => {
    const leftExpiry = left.snapshot.paymentExpiresAt
      ? Date.parse(left.snapshot.paymentExpiresAt)
      : Date.parse(nowIso);

    const rightExpiry = right.snapshot.paymentExpiresAt
      ? Date.parse(right.snapshot.paymentExpiresAt)
      : Date.parse(nowIso);
